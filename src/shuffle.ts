import type { Archetype, Blueprint, Child, MoodName, Section, ShuffleOptions } from './types.js'
import { type Rng, mulberry32, pick, int, chance } from './rng.js'
 ...
 */
const ROW_JUSTIFY = ['space-between', 'flex-start', 'flex-end', 'center'] as const
/** never 'stretch' next to text — stretched text blocks are the squished look we exist to avoid */
const ROW_ALIGN_TEXT = ['flex-end', 'baseline', 'flex-start', 'center'] as const
const COLUMN_ALIGN = ['flex-start', 'center', 'flex-end'] as const
const IMAGE_ASPECTS = ['4 / 3', '3 / 4', '1 / 1', '16 / 10', '2 / 3'] as const
const GAP_SCALES = [0.5, 1, 1, 1.5, 2] as const
...
function headline(rng: Rng, flex: string): Child {
  return { kind: 'headline', flex, content: pick(rng, HEADLINES) }
}
function paragraph(rng: Rng, flex: string): Child {
  return { kind: 'paragraph', flex, content: pick(rng, PARAGRAPHS) }
}
function caption(rng: Rng, flex: string): Child {
  return { kind: 'caption', flex, content: pick(rng, CAPTIONS) }
}
function image(ctx: Ctx, flex: string): Child {
  return {
    kind: 'image',
    flex,
...
export const ARCHETYPES: Record<Archetype, Generator> = {
  hero({ rng, withImages, hue }) {
    const kids: Child[] = [headline(rng, '1 1 55%')]
    if (withImages) kids.push(image({ rng, withImages, hue }, '0 1 32%'))
    else kids.push(caption(rng, '0 1 20%'))
    if (chance(rng, 0.5)) kids.unshift({ kind: 'caption', flex: '1 1 100%', content: pick(rng, KICKERS) })
    return {
      archetype: 'hero',
      display: 'flex',
      direction: 'row',
      justify: 'space-between',
      align: pick(rng, ['flex-end', 'baseline'] as const),
      gapScale: 1,
  ...
  split(ctx) {
    ...
    const kids: Child[] = [paragraph(rng, '1 1 34%')]
  ...
  stack({ rng }) {
    const kids: Child[] = [headline(rng, '0 1 auto')]
    for (let i = int(rng, 1, 2); i > 0; i--) kids.push(paragraph(rng, '0 1 auto'))
    return {
      archetype: 'stack',
      display: 'flex',
      direction: 'column',
  ...
  strip(ctx) {
    const { rng } = ctx
    const kids: Child[] = []
    for (let i = int(rng, 3, 5); i > 0; i--) kids.push(image(ctx, `1 1 ${int(rng, 16, 26)}%`))
    if (chance(rng, 0.6)) kids.push(caption(rng, '1 1 100%'))
    return {
      archetype: 'strip',
      display: 'flex',
      direction: 'row',
      justify: 'flex-start',
      align: 'flex-start',
      gapScale: pick(rng, [0.5, 1] as const),
      children: kids,
    }
  },
  collage(ctx) {
  ...
  masthead({ rng }) {
    return {
      ...
      justify: 'center',
      align: pick(rng, COLUMN_ALIGN),
      gapScale: pick(rng, [1, 1.5, 2] as const),
...
const IMAGE_ARCHETYPES: ReadonlySet<Archetype> = new Set(['strip', 'collage', 'full-bleed'])
/** Archetypes whose generators always emit a `headline` child — the only ones fit to open (they host the sole h1). */
const HEADLINE_OPENERS: readonly Archetype[] = ['hero', 'stack', 'collage', 'masthead', 'offset-stack']

export function shuffle(opts: ShuffleOptions = {}): Blueprint {
  const seed = opts.seed ?? 1
  const rng = mulberry32(seed)
  const mood: MoodName = !opts.mood || opts.mood === 'auto' ? pick(rng, ['gallery', 'editorial', 'zine'] as const) : opts.mood
  const withImages = opts.withImages ?? true
  const hue = int(rng, 0, 359)
  const ctx: Ctx = { rng, withImages, hue }

  // hero is opener-only: kept out of the rotation pool, offered only in the opener set.
  let pool = Object.keys(ARCHETYPES).filter((a) => a !== 'hero') as Archetype[]
  ...
  const sections: Section[] = [ARCHETYPES[opener](ctx)]
  let prev: Archetype = opener
...
export function validate(bp: Blueprint): string[] {
  const errors: string[] = []
  if (bp.sections.length === 0) errors.push('blueprint has no sections')
  ...
  bp.sections.forEach((s, i) => {
    if (s.children.length === 0) errors.push(`section ${i} (${s.archetype}) has no children`)
